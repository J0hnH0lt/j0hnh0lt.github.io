...
const LIM_ANCHOR  =13;    // Keep specified particle(s) at world-space location
                          // xMin, yMin, zMin.
const LIM_SLOT    =14;    // Limit specified particles(s) positions to stay
...
const LIM_ROD     =15;    // Connects 2 particles with fixed-length separation
                          // between particles whose indices are held in e1,e2
                          // (e.g. particles at pS0[e1] and pS0[e2] )
const LIM_ROPE    =16;    // Prevent 2 particles selected by members e1,e2 from 
                          // separating by more than distance 'radMax';
const LIM_RADIUS  =17;    // Prevent any particle in a set (targFirst,targCount)
...
const LIM_PULLEY  =18;    // Keep constant sum-of-distances for 3 particles
                          // A,B,Pivot:  ||A-Pivot||+||B-Pivot|| = dmax.
const LIM_MAXVAR  =19;    // Max number of possible limitType values available.
...
// Constructor for a new 'limit' object -- a constraint-applying object.
  ...
  this.anchor = 0; // particle number of anchor
  this.anchorPos = [0, 0, 0]; // permanent position

  this.spherePos = [0, 0, 0];
  this.sphereRad = 1;
}
